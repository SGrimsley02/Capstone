...
module UIHelpers {
    ...
    function drawBranding(dc as Graphics.Dc, cx as Number, H as Number, logo as Graphics.BitmapReference?) as Void {
        if (logo != null) {
            ...
            dc.drawBitmap2(cx - logoW / 2, (H * 0.04).toNumber(), logo, {:tintColor => ThemeHelpers.getColor("logo")}); // PURPLE_LITE
        } else {
            dc.setColor(ThemeHelpers.getColor("logo"), Graphics.COLOR_TRANSPARENT);
    ...
    function drawClock(dc as Graphics.Dc, cx as Number, y as Number, font as Graphics.FontDefinition) as Void {
        ...
        dc.setColor(ThemeHelpers.getColor("time"), Graphics.COLOR_TRANSPARENT);
        dc.drawText(cx, y, font, timeStr, Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);

        if (use12h && amPm.length() > 0) {
            dc.setColor(ThemeHelpers.getColor("date_am_pm"), Graphics.COLOR_TRANSPARENT); // GRAY_MID
            dc.drawText(
